...
def ista_block(input_layers, input_data, layer_no):
    ...
    var_list=[Weights0,Weights1,Weights11,Weights2,Weights22,Weights3]
...
def inference_ista(input_tensor, n, X_output, reuse):
    ...
    for i in range(n):
        with tf.variable_scope('conv_%d' %i, reuse=reuse):
            [conv1, conv1_sym],all_var = ista_block(layers, X_output, i)
            layers.append(conv1)
            layers_symetric.append(conv1_sym)
            var_list.append(all_var)
    return [layers, layers_symetric],var_list


[Prediction, Pre_symetric],Var_list = inference_ista(X0, PhaseNumber, X_output, reuse=False)

# cost0 = tf.reduce_mean(tf.square(Prediction - X_output))
...
saver.save(sess, './%s/CS_Saved_Model_%d.cpkt' % (model_dir, epoch_i), write_meta_graph=True,global_step=10)
...
dicts={'conv_0':Var_list[0],'conv_1':Var_list[1],'conv_2':Var_list[2],
      'conv3':Var_list[3],'conv_4':Var_list[4],'conv_5':Var_list[5],
      'conv_6':Var_list[6],'conv_7':Var_list[4],'conv_8':Var_list[5],
      }
modelP_path = os.path.join('.', 'params_dict.npy')
np.save(modelP_path, dicts)
sess.close()