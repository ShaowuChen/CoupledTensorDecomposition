...
def resnet18(in_tensor, is_training):
    ...
    weight_fc =  tf.get_variable(shape=[512, 100],  dtype=tf.float32, name='fc.weight',  initializer=initializer,regularizer=regularizer)
    bias_fc = tf.get_variable(shape=[100],  dtype=tf.float32, name='fc.bias', initializer=initializer,regularizer=regularizer)
...
flags = tf.flags
FLAGS=flags.FLAGS
flags.DEFINE_string('epoch', '250', 'train epoch' )
...
regularizer = tf.contrib.layers.l2_regularizer(1e-4) #L2 Regularizer
...
with tf.Session(config=config) as sess:
    ...
    with open(log_path, 'a') as f:
        f.write('time:2020/9/21  Reset10 for Cifar100 \n\n')
    ...
    for j in range(epoch):
        ...
        if (j+1)%5==0:
            ...
            saver.save(sess, ckpt_root_path+'/resnet10-acc'+str(test_acc)[0:7], global_step=j+1)
