...
flags.DEFINE_string('ckpt_path', '20200622', 'rank_rate of each block1 conv2')
...
def get_dict():
    ...
    ave_checkpoint_path = '/home/test01/csw/resnet34/resnet34_decom/joint_decom_alg1/W_ave/ckpt_ave/7_acc0.64157-7'
    ave_dict = {}
    ave_reader = pywrap_tensorflow.NewCheckpointReader(ave_checkpoint_path)
    ave_var_to_shape_map = ave_reader.get_variable_to_shape_map()

    for key in ave_var_to_shape_map:

        ave_dict[key] = ave_reader.get_tensor(key)
...
def iter(value_dict,rank_rate_ave,rank_rate_single):
    ...
    for i in range(3):
        ...
        for conv_num in [1,2]:
            ...
            locals()['layer' + str(layer_num) + '.conv' + str(conv_num) + '.apdate_W_ave_temp' ] = 0

            G3_ave, G2_ave, G1_ave = decompose(value_dict[ave_name], rank_rate_ave)

            locals()[ave_name + str(1)] = G1_ave
            locals()[ave_name + str(2)] = G2_ave
            locals()[ave_name + str(3)] = G3_ave
    ...
    for iters in range(11):
        ...
        for i in range(3):  #layer
            ...
            for j in range(repeats[i] - 1):  #repeat
                ...
                for conv_num in [1, 2]:   #conv
                    ...
                    locals()[diff_name] = value_dict[orig_name] - reverse_decompose( locals()[ave_name + '3'],   locals()[ave_name + '2'],  locals()[ave_name + '1'])
                    locals()[diff_name+'3'], locals()[diff_name+'2'], locals()[diff_name+'1'] = decompose( locals()[diff_name], rank_rate_single)


                    locals()['layer' + str(layer_num) + '.conv' + str(conv_num) + '.apdate_W_ave_temp'] += value_dict[orig_name]  - reverse_decompose(locals()[diff_name+'3'], locals()[diff_name+'2'], locals()[diff_name+'1'])
        ...
        for i in range(3):  # layer
            ...
            for conv_num in [1, 2]:  # conv
                    ...
                    locals()[ave_name] = value_dict[ave_name] + locals()['layer' + str(layer_num) + '.conv' + str(conv_num) + '.apdate_W_ave_temp']/(repeats[i]-1)
