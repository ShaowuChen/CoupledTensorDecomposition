...
if eval(FLAGS.whole_ave):
    ckpt_path = './ckpt_finetuned0.64_ave_20200622_lr0.0001/whole_ave/single_rate'+str(FLAGS.rank_rate_single[0])+'_'+str(FLAGS.rank_rate_single[2])
else:
    ckpt_path = './ckpt_finetuned0.64_ave_20200622_lr0.0001/Both_decom/yellow_rate'+str(FLAGS.rank_rate_yellow[0])+'_'+str(FLAGS.rank_rate_yellow[2])+'_ave_rate'+str(FLAGS.rank_rate_ave[0])+'_'+str(FLAGS.rank_rate_ave[2])
...
val_images, val_labels = image_processing.inputs(imagenet_data_val, batch_size=256, num_preprocess_threads=4)
...
def basic_conv_ave(in_tensor, value_dict, layer_num, conv_num, strides,   rank_rate_ave=None,  whole_ave=False):
    ...
    return tensor_go_next, G3,G2,G1


def basic_conv_single(in_tensor, value_dict, layer_num, repeat_num, conv_num,strides, rank_rate_single=None,G_ave3=None,G_ave2=None,G_ave1=None):
    ...
    if repeat_num==0 and conv_num==2:
        ...
        G3, G2, G1 = decompose(value_dict[name_weight], rank_rate)
...
def basic_conv(in_tensor, value_dict, layer_num, repeat_num, conv_num,
    ...
    if layer_num==1 or (repeat_num==0 and conv_num==1):
        ...
        if layer_num!=1 and repeat_num == 0 and conv_num == 2:#除layer1外， 其余layer的block1的第二层单独分解
            ...
            tensor_from_ave, G_ave3, G_ave2, G_ave1 = basic_conv_ave(in_tensor, value_dict, layer_num, conv_num,strides=strides,  rank_rate_ave=rank_rate_ave,  whole_ave=whole_ave)
            print('basic_conv'+str(type(G_ave3)))
            tensor_from_single = basic_conv_single(in_tensor, value_dict, layer_num, repeat_num, conv_num,strides=strides, rank_rate_single=rank_rate_single,G_ave3=G_ave3,G_ave2=G_ave2,G_ave1=G_ave1)
...
value_dict = np.load('./Diff_and_Ave_and_Orig.npy', allow_pickle=True).item()
...
with tf.Session(config = config) as sess:
    ...
    for j in range(eval(FLAGS.epoch)):
        ...
        saver.save(sess, ckpt_path+'/ckpt', global_step=j)
